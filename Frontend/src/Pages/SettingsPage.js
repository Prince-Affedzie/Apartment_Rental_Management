...
//   useEffect(() => {
//     if (profile?.role !== "Admin") return; // ✅ Prevent managers/staff from loading users
//     const fetchAllUsers = async () => {
//       setIsLoading((prev) => ({ ...prev, fetchUsers: true }));
//       try {
//         const response = await getAllUsers();
...
//     setIsLoading((prev) => ({
//       ...prev,
//       deleteUser: { ...prev.deleteUser, [Id]: true },
//     }));
//     try {
//       const response = await removeUser(Id);
//       if (response.status === 200) {
...
import { getAllUsers, addNewUser, updateUser, removeUser } from "../APIS/APIS";
import "react-toastify/dist/ReactToastify.css";
import { PlusCircle, Trash2, Pencil, Eye, EyeOff, Loader2 } from "lucide-react";
                    ...
                    {users.map((user) => (
                      ...
                      >
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() => setEditUser(user)}
                            className="px-3 py-1 bg-yellow-500 text-white rounded-md flex items-center gap-1"
                          >
                            <Pencil size={14} /> Edit
                          </button>
                          <LoadingButton
